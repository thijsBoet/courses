...
class Ship:
    ...
    def shoot(self):
        global a_missile
        forward = angle_to_vector(self.angle)
        missile_position = [self.pos[0] + self.radius *
                            forward[0], self.pos[1] + self.radius * forward[1]]
        missile_velocity = [self.vel[0] + 3 *
                            forward[0], self.vel[1] + 3 * forward[1]]
        a_missile = Sprite(missile_position, missile_velocity,
...
def rock_spawner():
    global a_rock
    a_rock = Sprite([random.randrange(WIDTH), random.randrange(HEIGHT)], [random.choice([0.5, -1]) * random.random() * 2,
                                                                          random.choice([0.5, -1]) * random.random() * 2], 1, random.choice([0.5, -1]) * 0.05, asteroid_image, asteroid_info)
...
a_rock = Sprite([WIDTH / 3, HEIGHT / 3], [1, 1], 0,
                0, asteroid_image, asteroid_info)
